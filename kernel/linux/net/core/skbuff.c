...
#endif
#if defined(CONFIG_MIPS_BRCM)
	memcpy(n->extif, skb->extif, sizeof(skb->extif));
	C(retfreeq_flags);
	n->retfreeq_flags &= ~SKB_PREALLOC;
	C(retfreeq_cb);
	C(retfreeq_context);
	C(rcvfrom);
	n->nat_cache_hit = n->nat_cache_rule = 0;
	n->nat_cache_modify = n->nat_cache_add = NULL;    
#endif
	C(truesize);
...
#endif
#if defined(CONFIG_MIPS_BRCM)
	new->rcvfrom 	= old->rcvfrom;
	new->retfreeq_cb = NULL;
	new->retfreeq_context = NULL;
	new->retfreeq_flags = 0;
    /* cloned/new skbuff does not inherit old skbuff's NAT characteristics */
	new->nat_cache_hit = new->nat_cache_rule = 0;
	new->nat_cache_modify = new->nat_cache_add = NULL;    
#endif
