   ...
   const void *data, unsigned int datalen)
{
   ...
   //dumpHexData((*pskb)->data, ((*pskb)->len < 48) ? (*pskb)->len : 48);
	...
	if ((*pskb)->protocol == __constant_htons(ETH_P_8021Q))
      ...
      iph = (struct iphdr *)((*pskb)->nh.raw + PPPOE_HDR_OFFSET);
   else
	   iph = (*pskb)->nh.iph;
	...
	if ((ftosinfo->ftos_set & FTOS_SETFTOS) && (iph->tos != ftosinfo->ftos)) {
      //printk("ebt_target_ftos:FTOS_SETFTOS .....\n");
	   ...
	   //printk("ebt_target_ftos:FTOS_WMMFTOS .....0x%08x\n", (*pskb)->nfmark & 0xf);
        ...
        diffs[0] = htons(iph->tos) ^ 0xFFFF;
	    iph->tos |= (((*pskb)->nfmark >> PRIO_LOC_NFMARK) & PRIO_LOC_NFMASK) << DSCP_MASK_SHIFT;
		diffs[1] = htons(iph->tos);
		iph->check = csum_fold(csum_partial((char *)diffs,
		                                    sizeof(diffs),
		                                    iph->check^0xFFFF));
