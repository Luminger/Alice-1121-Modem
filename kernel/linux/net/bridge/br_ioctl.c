...
static void addr_debug(unsigned char *dest)
{
#define NUM2PRINT 50
...
static void dolist(struct net_bridge *br)
{
	...
	list_for_each_rcu(lh, &br->mc_list) {
	    ...
	    printk("%s %06s    ", br->dev->name, dst->dst->dev->name);
...
static int br_set_port_snooping(struct net_bridge_port *p,  void __user * userbuf)
{
    ...
    br_mc_fdb_add(p->br, p, tmp, tmp+6, tmp[12], &src);
    return 0;
}
...
static int br_clear_port_snooping(struct net_bridge_port *p,  void __user * userbuf)
{
	...
	src.s_addr = *(unsigned int *)(tmp+13);
	br_mc_fdb_remove(p->br, p, tmp, tmp+6, tmp[12], &src);
    }
    return 1;
}
#endif
