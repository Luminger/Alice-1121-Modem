...
#if defined(CONFIG_MIPS_BRCM)
			// Call again in case no rules matched,
			// We'll add a default rule in that case
			if (brcm_nat_cache_hit_hook && (skb)->nat_cache_add != NULL)
				(skb)->nat_cache_add(NULL,NULL,skb);
#endif
...
#if defined(CONFIG_MIPS_BRCM)
	if (brcm_nat_cache_hit_hook && skb->nat_cache_hit == 1) {
		if (skb->nat_cache_modify) skb->nat_cache_modify(skb);
