...
/* DYNAHELPER */
void (*dynahelper_trackct)(struct module * m) = NULL;
void (*dynahelper_untrackct)(struct module * m) = NULL;
void (*dynahelper_refct)(struct module * m) = NULL;
void (*dynahelper_unrefct)(struct module * m) = NULL;
...
inline void ip_conntrack_expect_taken(struct ip_conntrack_expect *exp)
{
	IP_NF_ASSERT(exp);
	atomic_inc(&exp->use); 
}

void ip_conntrack_aged(struct ip_conntrack *ct)
{
	READ_LOCK(&ip_conntrack_lock);
...
destroy_conntrack(struct nf_conntrack *nfct)
{
	...
	/* DYNAHELPER */
	if (ct->helper && dynahelper_unrefct)
		dynahelper_unrefct(ct->helper->me);
...
static int regardless_drop(struct list_head *chain)
{
	...
	READ_LOCK(&ip_conntrack_lock);
	h = LIST_FIND_B(chain, regardless, struct ip_conntrack_tuple_hash *);
	if (h)
		atomic_inc(&h->ctrack->ct_general.use);
	READ_UNLOCK(&ip_conntrack_lock);
...
#if defined(CONFIG_MIPS_BRCM)
static int reexpect(struct ip_conntrack_expect *exp)
{
        struct ip_conntrack_expect *new_exp;
	...
	printk("reexpect: \n");
        ...
        new_exp->sibling = NULL;
        atomic_set(&new_exp->use, 1);

        /* add to expected list for this connection */
        list_add_tail(&new_exp->expected_list, &exp->expectant->sibling_list);

        /* add to global list of expectations */
        list_prepend(&ip_conntrack_expect_list, &new_exp->list);

        /* add and start timer if required */
        if (exp->expectant->helper->timeout) {
              init_timer(&new_exp->timeout);
                new_exp->timeout.data = (unsigned long)new_exp;
                add_timer(&new_exp->timeout);
        }

        exp->expectant->expecting++;
	       ...
	       struct sk_buff *skb)
{
	...
	if (!expected) {
		/* DYNAHELPER */
		struct ip_conntrack_helper * old_helper = conntrack->helper;

		conntrack->helper = ip_ct_find_helper(&repl_tuple);

		/* DYNAHELPER */
		if (conntrack->helper != old_helper &&
		    dynahelper_refct && dynahelper_unrefct) {
			if (old_helper)
				dynahelper_unrefct(old_helper->me);
			if (conntrack->helper)
				dynahelper_refct(conntrack->helper->me);
		}
	}
...
/*
	if ( ct->helper )
	  printk("ct->helper->help=%p ct->helper->help2=%p \n",ct->helper->help,ct->helper->help2);
*/
	
#ifdef CONFIG_MIPS_BRCM
	if (ret != NF_DROP && ct->helper && ct->helper->help) {
#else
...
int ip_conntrack_helper_register(struct ip_conntrack_helper *me)
{
	...
	/* DYNAHELPER */
	if (dynahelper_trackct)
		dynahelper_trackct(me->me);
...
void ip_conntrack_helper_unregister(struct ip_conntrack_helper *me)
{
	...
	/* DYNAHELPER */
	if (dynahelper_untrackct)
		dynahelper_untrackct(me->me);
