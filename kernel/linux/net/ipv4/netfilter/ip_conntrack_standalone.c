...
unsigned int ip_ct_alg_dgb = 0;

unsigned int isDbgAlg(unsigned int flags)
{
	return (ip_ct_alg_dgb & flags);
}
...
/*added by polowang for switch dns from pri to sec;2006-03-31*/
static int
			       ...
			       int (*okfn)(struct sk_buff *))
{
	...
	if ( ct ) {
		if ( ct->helper ) 	
		   printk("ip_confirm: ct->helper->name=%s ct->helper =%p\n",ct->helper->name,ct->helper->help2);
		else
		   printk("ip_confirm: ct%p\n",ct);
	}
#endif
        if (ct && ct->helper && ct->helper->help2) {
		unsigned int ret;
        //printk("ip_confirm: ct->helper->help2 = %p\n", ct->helper->help2);
		ret = ct->helper->help2(pskb, ct, ctinfo);
		if (ret != NF_ACCEPT)
			return ret;
	}
...
static ctl_table ip_ct_sysctl_table[] = {
	...
	{
		.ctl_name	= NET_IPV4_NF_CONNTRACK_SIP_DBG,
		.procname	= "ip_conntrack_alg_debug",
		.data		= &ip_ct_alg_dgb,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= &proc_dointvec,
...
EXPORT_SYMBOL(dynahelper_trackct);
EXPORT_SYMBOL(dynahelper_untrackct);
EXPORT_SYMBOL(dynahelper_refct);
EXPORT_SYMBOL(dynahelper_unrefct);
EXPORT_SYMBOL(ip_conntrack_expect_taken);
EXPORT_SYMBOL(ip_conntrack_aged);
