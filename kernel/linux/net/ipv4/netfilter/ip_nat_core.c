...
/* DYNAHELPER */
void (*dynahelper_refnat)(struct module * m) = NULL;
void (*dynahelper_unrefnat)(struct module * m) = NULL;
...
static void ip_nat_cleanup_conntrack(struct ip_conntrack *conn)
{
	...
	/* DYNAHELPER */
	if (info->helper && dynahelper_unrefnat)
		dynahelper_unrefnat(info->helper->me);
		  ...
		  unsigned int hooknum)
{
	...
	if (!conntrack->master) {
		/* DYNAHELPER */
		struct ip_nat_helper * old_helper = info->helper;

		info->helper = LIST_FIND(&helpers, helper_cmp, struct ip_nat_helper *,
					 &reply);

		/* DYNAHELPER */
		if (info->helper != old_helper &&
		    dynahelper_refnat && dynahelper_unrefnat) {
			if (old_helper)
				dynahelper_unrefnat(old_helper->me);
			if (info->helper)
				dynahelper_refnat(info->helper->me);
		}
	}
	    ...
	    struct sk_buff **pskb)
{
	...
	for (i = 0; i < info->num_manips; i++) {
		if (info->manips[i].direction == dir
			...
			extern void *brcm_nat_cache_hit_hook;
			if (info->helper)
				(*pskb)->nat_cache_add = NULL;
			if (brcm_nat_cache_hit_hook && (*pskb)->nat_cache_add)
				(*pskb)->nat_cache_add(ct,&info->manips[i],*pskb);
//			else printk("nat_cache_add NOT Set !\n");
#endif

			DEBUGP("Mangling %p: %s to %u.%u.%u.%u %u %u\n",
			       *pskb,
			       info->manips[i].maniptype == IP_NAT_MANIP_SRC
			       ? "SRC" : "DST",
			       NIPQUAD(info->manips[i].manip.ip),
                               info->manips[i].manip.u.tcp.port,
