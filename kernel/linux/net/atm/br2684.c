...
#if defined(CONFIG_BCM_VDSL)
static void do_8023_oam_processing( struct br2684_vcc *brvcc,
    PATM_VCC_DATA_PARMS pDp, int is_slow_proto );
#endif
    ...
    if (skb->len < MIN_PKT_SIZE)
    {
		...
		*/
		if (brvcc->vlan_id != 0xffff) {
			...
			//}
			// Keven -- begin
			if (skb->nfmark & 0x00000f00) {
				brvcc->vlan_id |= (skb->nfmark & 0x00000f00)<<5;
			}
			// Keven -- end            
    ...
    PATM_VCC_DATA_PARMS pDp, struct br2684_vcc *brvcc )
{
    ...
    if( !g_nPtmTraffic )
    {
       ...
       /* 2 byte of heade padding will appear for PTM traffic, which causes the
        * routed packets to be aligned on a 32-word boundary.
        * we just need to ignore it here for processing.
        */
        databuf += PTM_PADLEN;
        datalen -= PTM_PADLEN;
        dataoffset += PTM_PADLEN;

        const unsigned char slow_proto[] = {0x01,0x80,0xc2,0x00,0x00,0x02};
        int is_slow_proto = CMP_MAC_ADDR(slow_proto, databuf);
        if( brvcc->ptm_8023_oam_loopback_mode || is_slow_proto )
        {
            do_8023_oam_processing( brvcc, pDp, is_slow_proto );
            return;
        }
    }
#endif
    ...
    if( skb )
    {
...
#if defined(CONFIG_BCM_VDSL)
        /* If this is a VLAN tagged 802.1ag packet, change the MAC type so
         * the packet is processed by the 802.1ag packet handler.
         */
        if( g_nPtmTraffic && skb->protocol == ETH_P_8021Q &&
            *(unsigned short *) &skb->data[2] == ETH_P_8021AG )
        {
            skb->protocol = ETH_P_8021AG;
        }
#endif
        ...
        else
        {
            ...
            if(list_empty(&brdev->net_devexts)) {
                ...
                else if (0 == bcm_is_pppoe_passthrough_pkt(brdev, skb, &net_dev_match)) {
                    list_for_each(lh, &brdev->net_devexts) {
                        net_dev_ext = list_entry_devext(lh);
                        devext = EXTPRIV(net_dev_ext);
                        skb2 = skb_copy(skb, GFP_ATOMIC);
...
#if defined(CONFIG_BCM_VDSL) || defined(PTM_ETH_FCS)
/*******************************************************************************
 * crc32
 *
...
static unsigned long crc32(unsigned char *pdata, unsigned int nbytes,
    unsigned long crc)
{
    while (nbytes-- > 0)
...
/*******************************************************************************
 * 802.3ah Ethernet OAM Loopback Mode
 *******************************************************************************/

#define OAMPDU_CODE_INFO            0
#define OAMPDU_CODE_LOOPBACK        4

#define TLV_LOCAL_INFO              1
#define TLV_REMOTE_INFO             2

#define INFO_OAM_VERSION            1
#define INFO_STATE_LOOPBACK         0x05
#define INFO_OAM_CFG_LB             0x04

#define OAM_LB_ENABLE               1
#define OAM_LB_DISABLE              2

typedef struct oampdu
{
    unsigned char oampdu_da[ETH_ALEN];
    unsigned char oampdu_sa[ETH_ALEN];
    unsigned short oampdu_type;
    unsigned char oampdu_subtype;
    unsigned char oampdu_flags[2];
    unsigned char oampdu_code;
} OAMPDU, *POAMPDU;

typedef struct tlv_info
{
    unsigned char ti_type;
    unsigned char ti_length;
    unsigned char ti_oam_version;
    unsigned char ti_revision[2];
    unsigned char ti_state;
    unsigned char ti_oam_cfg;
    unsigned char ti_oampdu_cfg[2];
    unsigned char ti_oui[3];
    unsigned char ti_vendor_info[4];
} TLV_INFO, *PTLV_INFO;


/**************************************************************************
 * Function Name: create_info_oampdu
 * Description  : Creates an Information OAMPDU to send from a received
 *                Information OAMPDU.
 * Returns      : None.
 ***************************************************************************/
static void create_info_oampdu(struct br2684_vcc *brvcc, PATM_VCC_DATA_PARMS pDp,
    POAMPDU pOp)
{
    unsigned char *p = (unsigned char *) (pOp + 1);
    PTLV_INFO t = (PTLV_INFO) p;
    PATM_BUFFER pAb = pDp->pAtmBuffer;
    unsigned long crc;

    /* Set source MAC address. */
    memcpy(pOp->oampdu_sa, brvcc->device->dev_addr, ETH_ALEN);

    pAb->ulDataLen = sizeof(OAMPDU);
    if( pOp->oampdu_code == OAMPDU_CODE_INFO && t->ti_type == TLV_LOCAL_INFO )
    {
        int i;

        /* Move received Local Information TLV to the Remote Information TLV. */
        t->ti_type = TLV_REMOTE_INFO;
        for( i = t->ti_length - 1; i >= 0; i-- )
             p[i + sizeof(TLV_INFO)] = p[i];
        pAb->ulDataLen += t->ti_length;
    }

    /* Create Local Information TLV. */
    t->ti_type = TLV_LOCAL_INFO;
    t->ti_length = sizeof(TLV_INFO);
    t->ti_oam_version = INFO_OAM_VERSION;
    t->ti_revision[0] = 0;
    t->ti_revision[1] = 0;
    t->ti_state = (brvcc->ptm_8023_oam_loopback_mode) ? INFO_STATE_LOOPBACK : 0;
    t->ti_oam_cfg = INFO_OAM_CFG_LB;
    t->ti_oampdu_cfg[0] = (ETH_FRAME_LEN + 4) >> 8;     /* +4 for FCS length */
    t->ti_oampdu_cfg[1] = (ETH_FRAME_LEN + 4) & 0xff;
    t->ti_oui[0] = brvcc->device->dev_addr[0];
    t->ti_oui[1] = brvcc->device->dev_addr[1];
    t->ti_oui[2] = brvcc->device->dev_addr[2];
    t->ti_vendor_info[0] = t->ti_vendor_info[1] = 0;
    t->ti_vendor_info[2] = t->ti_vendor_info[3] = 0;
    pAb->ulDataLen += t->ti_length;

    if( pAb->ulDataLen < MIN_PKT_SIZE )
        pAb->ulDataLen = MIN_PKT_SIZE;

    crc = ~crc32(pAb->pDataBuf, pAb->ulDataLen, 0xffffffff);
    p = pAb->pDataBuf + pAb->ulDataLen;
    p[0] = (crc >>  0) & 0xff;
    p[1] = (crc >>  8) & 0xff;
    p[2] = (crc >> 16) & 0xff;
    p[3] = (crc >> 24) & 0xff;
    pAb->ulDataLen += 4;                                /* +4 for FCS length */
}

/**************************************************************************
 * Function Name: do_8023_oam_processing
 * Description  : Processes a packet whose destination address is the 802.3ah
 *                "slow protocol" multicast address or the VCC is in 802.3ah
 *                loopback mode.
 * Returns      : None.
 ***************************************************************************/
static void do_8023_oam_processing( struct br2684_vcc *brvcc,
...
{
    PATM_BUFFER pAb = pDp->pAtmBuffer;
    POAMPDU pOp = (POAMPDU) pAb->pDataBuf;
    int packet_created = 0;
    unsigned char oam_lb;

    if( is_slow_proto )
    {
        switch( pOp->oampdu_code )
        {
        case OAMPDU_CODE_INFO:
            create_info_oampdu( brvcc, pDp, pOp );
            packet_created = 1;
            break;

        case OAMPDU_CODE_LOOPBACK:
            oam_lb = *(unsigned char *) (pOp + 1);
            if( oam_lb == OAM_LB_ENABLE )
                brvcc->ptm_8023_oam_loopback_mode = 1;
            else
                brvcc->ptm_8023_oam_loopback_mode = 0;
            create_info_oampdu( brvcc, pDp, pOp );
            packet_created = 1;
            break;
    ...
    if( packet_created == 0 && brvcc->ptm_8023_oam_loopback_mode &&
        (pOp->oampdu_da[0] & 0x01) == 0x00 )
    {
        /* Loop packet back to sender. */
        unsigned char macaddr[ETH_ALEN];

        /* Swap source and destination MAC addresses. */
        memcpy(macaddr, pOp->oampdu_da, ETH_ALEN);
        memcpy(pOp->oampdu_da, pOp->oampdu_sa, ETH_ALEN);
        memcpy(pOp->oampdu_sa, macaddr, ETH_ALEN);
        packet_created = 1;
    }

    if( packet_created )
    {
        if( BcmAtm_SendVccData( brvcc->atm_attach_handle, pDp ) != STS_SUCCESS )
            (*pDp->pFnFreeDataParms) (pDp);
    }
    else
        (*pDp->pFnFreeDataParms) (pDp);
