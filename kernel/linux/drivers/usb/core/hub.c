...
extern void kerSysLedCtrl(int,int);
...
void usb_disconnect(struct usb_device **pdev)
{
  ...
  if ((&udev->dev)->bus_id[2] == '1') {
     kerSysLedCtrl(12, 0);
     kerSetUsbInfo(0,0,"未连接");
  }
  else if ((&udev->dev)->bus_id[2] == '2') {
  	 kerSysLedCtrl(13, 0);
     kerSetUsbInfo(1,0,"未连接");
  }
...
#define HUB_DEBOUNCE_TIMEOUT	15000
...
static int hub_port_debounce(struct usb_device *hdev, int port)
{
	...
	for (total_time = 0; ; total_time += HUB_DEBOUNCE_STEP) {
		...
		if (!(portchange & USB_PORT_STAT_C_CONNECTION) &&
			...
			if ((stable_time >= HUB_DEBOUNCE_STABLE) && (portstatus != old_portstatus))
			{
				old_portstatus = portstatus;
				break;
			}
		} else {
			stable_time = 0;
...
hub_port_init (struct usb_device *hdev, struct usb_device *udev, int port)
{
  ...
  if ((&udev->dev)->bus_id[2] == '1') {
     kerSysLedCtrl(12, 1);
     kerSetUsbInfo(0,1,"已连接");
  }
  else if ((&udev->dev)->bus_id[2] == '2') {
  	 kerSysLedCtrl(13, 1);
  	 kerSetUsbInfo(1,1,"已连接");
  }
