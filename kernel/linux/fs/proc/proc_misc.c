...
static int accu_meminfo_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	struct sysinfo i;
	int len, committed;
	struct page_state ps;
...
#define K(x) ((x) << (PAGE_SHIFT - 10))
	 ...
	 */
	len = sprintf(page,
		"Accurate free memory information - sampled after freeing pages with a full memory scan\n\n"
		"MemTotal:     %8lu kB\n"
		"MemFree:      %8lu kB\n"
		"Buffers:      %8lu kB\n"
		"Cached:       %8lu kB\n"
...
int pagewalk_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len;
...
int free_pagewalk_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len;
...
#ifdef CONFIG_BRCM_VMTOOLS
		{"accumem",	accu_meminfo_read_proc},
		{"pagewalk", pagewalk_read_proc},
		{"free_pagewalk", free_pagewalk_read_proc},
#endif		
		{NULL,}
	};
