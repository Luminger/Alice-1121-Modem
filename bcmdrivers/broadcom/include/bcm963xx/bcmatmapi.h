...
typedef UINT8 *AllocFunction(int requestedSize);
typedef void   FreeFunction(UINT8 *bufferp);


typedef struct MainBufferHeader MainBufferHeader;
struct MainBufferHeader
{
  MainBufferHeader *nextBuffer;
  UINT16 allocatedSmallBuffer;
  UINT16 dummy1;
  UINT16 dummy2;
  UINT16 applicationFirst;
  UINT16 applicationSecond;
};


#define MIN_MAIN_BUFFER_OFFSET 4
#endif
...
#ifdef BCM_ATM_BONDING_ETH
  AllocFunction *allocp; // if not NULL, will be used to allocate buffers
  FreeFunction  *freep;  // must be defined when allocp is not NULL 
  UINT16   mainBufferNumber; // indicates the number of buffer that should be allocated
  UINT16   mainBufferOffset; // application specific area (init with all 0) -
                             // aligned
#endif
...
BCMATM_STATUS BcmAtm_SendTransparentData( UINT32 ulHandle, UINT32 ulInterfaceId,
    PATM_VCC_DATA_PARMS pDataParms );
#ifdef BCM_ATM_BONDING_ETH
int BcmAtm_detachBuffer(UINT32 ulHandle, PATM_VCC_DATA_PARMS pDataParms );
#endif
#if defined(__cplusplus)
}
#endif
