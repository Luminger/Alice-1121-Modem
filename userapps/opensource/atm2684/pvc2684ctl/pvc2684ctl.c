...
// brcm
#define MAKE_NAS_IDX(PORT,VPI,VCI) \
    ((((unsigned long) (PORT) & 0xff)  << 24) | \
     (((unsigned long) (VPI) & 0xff)   << 16) | \
     (((unsigned long) (VCI) & 0xffff) <<  0))
...
// brcm
      {
        int retval, num[4];
        retval = sscanf(optarg, "%d.%d.%d.%d", num, num+1, num+2, num+3);
        starting_nas_idx = MAKE_NAS_IDX(num[0],num[1],num[2]);
      }
      break;
