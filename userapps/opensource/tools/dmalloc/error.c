...
void	_dmalloc_open_log(void)
{
  char	log_path[1024];
  int	len;
// BRCM add 2 lines below
  outfile_fd = 1;
  *dmalloc_logpath = (char *)"";
#if 0 // BRCM commented following
...
void	_dmalloc_vmessage(const char *format, va_list args)
{
  ...
  // BRCM  -- add this -- send message to syslog
  syslog(LOG_NOTICE,message_str);
...
#if 0  // BRCM commented following.  not to write to logfile
