...
#else
#ifdef BUILD_STATIC
snmp_main(int argc, char *argv[])
#else
main(int argc, char *argv[])
#endif
#endif
{
    int             isConfigFileProvided;
   ...
   isConfigFileProvided = 0;
    ...
    while ((arg = getopt(argc, argv, options)) != EOF) {
        ...
        case 'c':
            isConfigFileProvided = 1;
            snmp_log(LOG_WARNING, "optarg = %s\n", optarg);
            if (optarg != NULL) {
    ...
    if ( isConfigFileProvided == 0 )
    {
      /* If the config file was not provided on the
      ** command line, use the default SNMP configuration file*/
      netsnmp_ds_set_string(NETSNMP_DS_LIBRARY_ID,
				      NETSNMP_DS_LIB_OPTIONALCONFIG, "/var/snmpd.conf");
    }
...
#if 0
#ifdef BUILD_ILMI /* testing only */
    extern int autoconfig_init(char *pvcAddrStr);
#ifdef SNMP_TRANSPORT_AAL5PVC_DOMAIN
    /* defined in \include\net-snmp\net-snmp-config.h */
    autoconfig_init("pvc:0.0.16");
#else
    autoconfig_init("udp:192.168.1.10:162");
#endif
#endif /* BUILD_ILMI */
#endif /* 0 */
