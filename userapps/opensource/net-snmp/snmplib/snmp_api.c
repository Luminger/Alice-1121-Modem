...
#include <arpa/inet.h>
#include <stdarg.h>
static int snmp_access_filtered(netsnmp_transport *transport, void *data, int length);
...
#endif /* #ifndef BUILD_ILMI */
}

#ifdef BRCM_SNMP_DEBUG
void
...
#if (defined BUILD_ILMI) && (defined BRCM_SNMP_DEBUG)
    xdump(packet, length, "");
#endif
                     ...
                     u_char * packetptr, int length)
{
  ...
  /* brcm: do access filtering operation */
  if (snmp_access_filtered(transport, opaque, olength)) {
    DEBUGMSGTL(("sess_process_packet", "snmpAccessFiltered\n"));
    if (opaque != NULL) {
      free(opaque);
    }
    return -1;
...
#define REMOTE_ACCESS 1
#define ACL_SERVICES_FILE  "/var/services.conf"
#define ACL_IP_LIST_FILE   "/var/acl.conf"
#define LAN_DEVICE_NAME    "br0"
/* from cfm */
static int getLanInfo(char *lan_ifname, struct in_addr *lan_ip, struct in_addr *lan_subnetmask)
{
   ...
   if ((socketfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
     printf("snmp: Error openning socket when getting LAN info\n");
     return -1;
   }

   strcpy(lan.ifr_name,lan_ifname);
   if (ioctl(socketfd,SIOCGIFADDR,&lan) < 0) {
     printf("snmp: Error getting LAN IP address\n");
     close(socketfd);
     return -1;
   }
   *lan_ip = ((struct sockaddr_in *)&(lan.ifr_addr))->sin_addr;
   ...
   if (ioctl(socketfd,SIOCGIFNETMASK,&lan) < 0) {
     printf("snmp: Error getting LAN subnet address\n");
     close(socketfd);
     return -1;
   }
...
/* from cfm */
static int isAccessFromLan(struct in_addr clntAddr)
{
   struct in_addr inAddr, inMask;
   char wan[64];

   getLanInfo(LAN_DEVICE_NAME, &inAddr, &inMask);
...
static int isInAccessControlList(char *addr) {
  int in=0;
  char line[16];
  char aclAddr_string[16];
  struct in_addr inAddr, aclAddr;

  FILE* fs = fopen(ACL_IP_LIST_FILE, "r");
  ...
  if ( fs == NULL ) {
    ...
    if ( atoi(line) == 0 ) {
      ...
      while (fgets(line,16,fs)) {
	sscanf(line,"%s\n",aclAddr_string);
	inet_aton(aclAddr_string,&aclAddr);
	inet_aton(addr,&inAddr);
	if ((aclAddr.s_addr) == (inAddr.s_addr)) {
	  in = 1;
	  break;
	}
      } /* while */
...
static int isServiceAllow(char *name, int direction)
{
  int allow=0;
  char service[16];
  int lanAllow, wanAllow;

  FILE* fs = fopen(ACL_SERVICES_FILE, "r");
  ...
  if ( fs == NULL ) {
    ...
    // mode is enabled then access control list is active
    // separated by TAB, snmpTABlanAllowTABwanAllow */
    while ((fscanf(fs,"%s\t%d\t%d",service,&lanAllow,&wanAllow)) != EOF) {
      if (strncasecmp(service,name,strlen(name)) == 0) {
	if (direction == LOCAL_ACCESS)
	  allow = lanAllow;
	else if (direction == REMOTE_ACCESS)
	  allow = wanAllow;
	break;
      }
    }
...
static int snmp_access_filtered(netsnmp_transport *transport, void *data, int length) 
{
  ...
  int filter=0;
  int inFromLan;
  int allowLanAccess, allowWanAccess;
  struct in_addr inAddr;

  allowLanAccess = isServiceAllow("snmp",LOCAL_ACCESS);
  allowWanAccess = isServiceAllow("snmp",REMOTE_ACCESS);

  if (!allowLanAccess && !allowWanAccess)
    return 1;

  /* address cannot be determined */
  if (transport != NULL && transport->f_fmtaddr != NULL) {
    addr_string = transport->f_fmtaddr(transport, data,length);
    if (addr_string != NULL) {
      /* first, check if Lan Access or Wan address */
      inet_aton(addr_string,&inAddr);
      inFromLan = isAccessFromLan(inAddr);

      /* first check services list */
      if (inFromLan && !allowLanAccess)
	filter = 1;
      else if (!inFromLan) { /* from Wan */
	if (!allowWanAccess)
	  filter = 1;
      }
      /* second, if still not filtered, check IP address */
      if (!filter) {
	if (!isInAccessControlList(addr_string)) 
	  filter = 1;
      } /* !filter */
    } /* addr_string!=NULL */
  } /* fmtaddr != NULL */
