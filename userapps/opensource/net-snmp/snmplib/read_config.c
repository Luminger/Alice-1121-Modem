...
read_config_with_type(const char *filename, const char *type)
{
    ...
    if (ctmp)
        read_config_snmp(filename, ctmp, EITHER_CONFIG);
    else
...
Renamed to read_config_snmp to resolve linket error:
 read_config is redefined in /CommEngine/user96345/src/udhcp-0.9.6/udhcp.a
 See also "/net-snmp-5.0.3/include/net-snmp/library/read_config.h" 
 Anothe way to fix the problem is declare the function "static". It is used only in this file.
 --YD

 */
void
read_config_snmp(const char *filename,
                ...
                for (j = 0; j <= MAX_PERSISTENT_BACKUPS; j++) {
                    ...
                    if (stat(configfile, &statbuf) != 0) {
                        ...
                        DEBUGMSGTL(("read_config_files",
                                    ...
                                    configfile));
                        read_config_snmp(configfile, ltmp, when);
                    }
                }
            }
